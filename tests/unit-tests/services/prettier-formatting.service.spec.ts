...
describe("PrettierFormattingService", () => {
  ...
  let pluginLoaderMock: jest.Mocked<
    Pick<PrettierPluginLoaderService, "getParserAndPlugins">
  >;
  let settingsServiceMock: { editorSettings: ReturnType<typeof makeSettings> };

  beforeEach(() => {
    pluginLoaderMock = { getParserAndPlugins: jest.fn() };
  ...
  describe("formatCode — successful formatting", () => {
    beforeEach(() => {
      pluginLoaderMock.getParserAndPlugins.mockResolvedValue(
        makePluginResult("babel")
      );
      (prettier.format as jest.Mock).mockResolvedValue("const x = 1;\n");
    });
    ...
    it("should call prettier.format with the resolved parser and plugins", async () => {
      ...
      expect(prettier.format).toHaveBeenCalledWith(
        "const x=1",
        expect.objectContaining({ parser: "babel", plugins: [mockPlugin] })
      );
    });
    ...
    it("should pass tabWidth from settings to prettier", async () => {
      ...
      expect(prettier.format).toHaveBeenCalledWith(
        "code",
        expect.objectContaining({ tabWidth: 4 })
      );
    });
    ...
    it("should pass useTabs: true when indentation mode is Tabs", async () => {
      ...
      expect(prettier.format).toHaveBeenCalledWith(
        "code",
        expect.objectContaining({ useTabs: true })
      );
    });
    ...
    it("should pass useTabs: false when indentation mode is Spaces", async () => {
      ...
      expect(prettier.format).toHaveBeenCalledWith(
        "code",
        expect.objectContaining({ useTabs: false })
      );
    });

    it("should spread pluginOptions into the prettier.format call", async () => {
      pluginLoaderMock.getParserAndPlugins.mockResolvedValue(
        makePluginResult("json", { jsonRecursiveSort: true })
      );
      ...
      expect(prettier.format).toHaveBeenCalledWith(
        "{}",
        expect.objectContaining({ jsonRecursiveSort: true })
      );
    });
  });
  ...
  describe("formatCode — JSON key sorting", () => {
    it("should include jsonRecursiveSort: true when formatting JSON", async () => {
      ...
      expect(prettier.format).toHaveBeenCalledWith(
        '{"z":2,"a":1}',
        expect.objectContaining({ jsonRecursiveSort: true, parser: "json" })
      );
  ...
  describe("formatCode — failure handling", () => {
    it("should return original code with formattingSuccessful: false when prettier throws", async () => {
      ...
      (prettier.format as jest.Mock).mockRejectedValue(
    ...
    it("should not throw when prettier format rejects", async () => {
      ...
      (prettier.format as jest.Mock).mockRejectedValue(new Error("oops"));
      const consoleSpy = jest
        .spyOn(console, "warn")
        .mockImplementation(() => {});

      await expect(
        service.formatCode("broken code", makeLanguage())
