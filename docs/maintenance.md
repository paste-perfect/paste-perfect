...
[Semantic-release](../.releaserc.json) uses Git tags as the release-version authority; there are no package-version commits to reconcile. The header displays and links to the tag from **deployment.json**. That file retains the source SHA for verification. History-only deployments retain the reachable release version; production excludes prereleases.
