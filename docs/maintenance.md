...
4. **Main push:** validate, publish a stable release when needed and deploy the production artifact. A checked reverse-sync PR brings main history back into dev; its branch is recreated whenever merging the previous sync PR deleted it.
...
| [auto-merge.yml](../.github/workflows/auto-merge.yml)                 | After PR validation or delivery, plus **06:15 and 18:15 UTC** retries. Refreshes outdated Renovate branches, maintains sync PRs and merges eligible, current, green PRs. A failed sync is reported without skipping those merges. |
...
[Semantic-release](../.releaserc.json) uses Git tags as the release-version authority; there are no package-version commits to reconcile. Dependency updates create patch releases; explicit breaking changes remain major releases. The header displays and links to the tag from **deployment.json**, which retains the source SHA for verification. History-only deployments retain the nearest release reachable **on the channel that target publishes**: production resolves stable tags only, preview resolves **rc** tags only. Neither may borrow the other's tag — a reverse sync parks the stable tag closer to dev than the last rc, so an unfiltered lookup would make the preview advertise the production release while serving different source. Preview falls back to a stable tag only when no prerelease is reachable at all.
